...
#[async]
fn change_request(io: (O,I), stun_server: SocketAddr, timeout: Duration, req: ChangeRequest)
    -> Result<((O, I), Option<Response>), ((O, I), std::io::Error)>
{
    ...
    await!(send_request(io, stun_server, timeout, req))
}

#[async]
fn send_request(io: (O, I), stun_server: SocketAddr, timeout: Duration, req: Request)
    -> Result<((O, I), Option<Response>), ((O, I), std::io::Error)>
{
    ...
    match await!(timer.timeout_stream(stream, timeout).into_future()) {
        Ok((Some((_id, resp)), t)) => {
            let stream = Box::new(t.into_inner());
